...
Please see table \@ref(tab:firstbenchmarkdf) below for all the models and their root mean squared errors (RMSE) on the validation set.
...
## Data Preprocessing
  We considered removing outliers in terms of 'dep_delay' in train but not in validation, then use k-folds cross validation on test to determine how many outliers we should remove to boost performance on the validation set. We considered removing highly influential points in order to train a better model. In this case, we consider highly influential points to be points with high Cook's distances.
