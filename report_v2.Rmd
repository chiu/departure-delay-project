...
* converting the shed_arr_time and sched_dep_time columns into the POSIX time format so that we can accurately take the difference of them.
...
The model in which we predicted the mean for all the predictions had an RMSE of 8.46926.
...
## Conclusion
 ...
 Out of the methods that we covered in class, we found gradient boosted models to provide the best performance based on having the lowest root mean squared error on the cross validations et. we believe that this makes sense because GBMs are able to capture non linear relationships between the explanatory variables and 'dep_delay' whereas linear regression cannot. 
