...
Please see table @\ref{tab:benchmarkdf} for all the models and their root mean squared error (RMSE) on the cross validation set.
...
filename <- "models/gbm_shrinkage_0point01_ntrees_1000_v1.rds"
saveRDS(model, filename)
rmse = sqrt(mean((test_df$dep_delay - preds)^2))
rmse
summary(model)
...
for (val in x) {
  ...
  print(val)
  print(rmse)
  filename <- paste0("performance/gbm_shrinkage_0point001_ntrees_", val)
...
num_trees_vs_rmse <- as.data.frame(num_trees=x, rmse=rmse_vec)
write.csv(num_trees_vs_rmse, 'performance/gbm_shrinkage_0point001_num_trees_vs_rmse_v1.csv')
...
write.csv(summary,'performance/gbm_shrinkage_0point001_16384trees_summary_v1.csv')
```
